...
public class MainGameLoop {
	...
	public static void main(String[] args) {
		...
		//Tree Models
		RawModel treeWoodModel = OBJLoader.loadObjModel("TreeWood2", loader);
		ModelTexture treeTexture = new ModelTexture(loader.loadTexture("treetestskin"));
		...
		for(int i=0;i<100;i++){
			...
			entities.add(new Entity(treeWoodStaticModel, random.nextInt(4),
					new Vector3f(xPos,yPos,zPos),0,0,0,size));
			entities.add(new Entity(treeLeavesStaticModel,
					new Vector3f(xPos,yPos,zPos),0,random.nextFloat() * 360,0,size));
		...
		lights.add(new Light(new Vector3f(0,1000,-7000), new Vector3f(0.4f,0,0)));
		lights.add(new Light(new Vector3f(0,10,0), new Vector3f(2,0,0), new Vector3f(1,0.01f,0.002f)));
		...
		firstPersonCameraPlayer = new FirstPersonCameraPlayer(cameraTexturedModel, new Vector3f(0, 0, 0), 0,0,0,2);
